...
4. Place the “Temporal workflow status” block on the node sidebar (Structure → Block layout) so editors can see workflow state.
...
- The Workflow Status block uses `/workflows/{id}/status` to render the current stage and pending locales.
- Editors can send signals via internal routes like `/temporal/signal/{nid}?signal=approvalGranted` (link these to buttons if desired); translation signals accept a `locale` query param.
...
4. Visit the node; the status block should show stage `translation`. Use the curl commands from the worker README to send translation/approval/publish signals and watch the block update after page refresh.
